...
.monitoring-page {
  ...
  height: 100%; /* Fill the main element's height */
  overflow: hidden; 
  box-sizing: border-box; 
}

/* Ensure Header doesn't shrink */
/* Assuming AppHeader component itself handles flex-shrink: 0 */
...
.container {
  ...
  flex: 1; /* Fill remaining space in monitoring-page */
  width: 100%;
  box-sizing: border-box;
  position: relative;
  overflow: hidden; 
...
.panel {
  ...
  min-height: 80px; /* Reduce min-height further if needed */
  box-sizing: border-box; /* 添加 */
}

.panel:hover {
  /* transform: translateY(-8px) scale(1.02); */ /* 保持移除放大效果 */
...
.video-container, .warning-content, .alerts-list-container, .qa-content {
  ...
  flex-grow: 1; /* Content area fills panel space */
  display: flex; 
  flex-direction: column; 
...
/* 特别为 AlertsPanel 启用滚动 */
.alerts-panel .alerts-list-container {
  overflow-y: auto; /* Enable vertical scroll */
  /* Scrollbar hidden globally in App.vue */
}
