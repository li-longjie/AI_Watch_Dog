...
def ensure_directories():
    ...
    for directory in required_dirs:
        try:
            ...
            logging.error(f"创建目录 {directory} 时出错: {e}")
            raise
...
async def video_feed(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            # 获取视频帧和行为检测结果
            frame_data, behavior_data = await video_processor.get_frame_and_behavior()
    ...
    <script>
        ...
        function connectWebSocket() {
            try {
                ...
                videoWs.onclose = function() {
                    ...
                    // 清空当前行为显示
                    document.getElementById('current-behavior').textContent = "当前行为: 未连接";
                };
        ...
        function updateBehaviorData(data) {
            ...
            if (!videoWs || videoWs.readyState !== WebSocket.OPEN) {
                console.log("WebSocket 连接已断开，不更新数据");
                return;
            }
            
            const behaviorNum = data.behavior;
        ...
        document.getElementById('refresh-btn').addEventListener('click', function() {
            ...
            document.getElementById('status-text').textContent = "数据已重置，等待新的检测结果";
        });
        ...
        document.getElementById('refresh-btn-stats').addEventListener('click', function() {
            ...
            // 更新状态显示
            document.getElementById('current-behavior').textContent = "当前行为: 等待检测...";
            document.getElementById('status-text').textContent = "统计数据已重置，等待新的检测结果";
        });
        ...
        const pieChart = new Chart(pieCtx, {
            ...
            plugins: [{
                ...
                beforeDraw: function(chart) {
                    ...
                    ctx.fillText('行为变化', centerX, centerY);
                    ctx.restore();
                }
            }]
        });
